...
| [`src/nvhttp.cpp`](../../src/nvhttp.cpp) | Duplicate `has_stream_session_activity()` defined under `#ifndef _WIN32` | Removed duplicate definition (upstream promoted it to file-level helper). |
...
| [`cmake/dependencies/Boost_Sunshine.cmake`](../../cmake/dependencies/Boost_Sunshine.cmake) & [`tests/CMakeLists.txt`](../../tests/CMakeLists.txt) | System Boost 1.90.0 on Ubuntu failed `find_package(Boost CONFIG 1.89.0 ...)` with header-only components, causing FetchContent conflict | Query only compiled Boost components and alias header-only targets from `Boost::headers`. |
| [`src_assets/linux/misc/vibepollo-mangohud`](../../src_assets/linux/misc/vibepollo-mangohud) | Dash `/bin/sh` does not support `[^...]` character set negation | Changed `*[^0-9.]*` to POSIX `*[!0-9.]*`. |
| [`src/platform/linux/capability_sanitizer.h`](../../src/platform/linux/capability_sanitizer.h) | Systemd user sessions pass `CAP_WAKE_ALARM` in inheritable set (`CapInh`), causing both unprivileged launch rejection and privileged exact-entry comparison failures (`EPERM`) | Clear inherited capabilities on Linux before checking context so unprivileged launches proceed without `no_new_privs` and privileged hosts retain exact permitted `{CAP_SYS_ADMIN, CAP_SYS_NICE}` sets. |
| [`cmake/packaging/linux.cmake`](../../cmake/packaging/linux.cmake) | Debian packages lacked runtime `libwebp` dependency for dynamic artwork conversion | Added `libwebp7 | libwebp6 | libwebp` to `CPACK_DEBIAN_PACKAGE_DEPENDS`. |
| [`src/steam_artwork.cpp`](../../src/steam_artwork.cpp) & [`cmake/compile_definitions/common.cmake`](../../cmake/compile_definitions/common.cmake) | Bundled FFmpeg lacks PNG/WebP/JPEG codecs; system missing `libwebp-dev`/`libjpeg-dev` | Pass through valid PNG directly; dynamically resolve WebP decoder from runtime `libwebp.so.7` via `dlopen`; link system `libpng` for encoding. |
| [`tests/unit/platform/linux/test_local_deploy.py`](../../tests/unit/platform/linux/test_local_deploy.py) & [`test_linux_installer.sh`](../../tests/unit/platform/linux/test_linux_installer.sh) | User session umask `0002` created group-writable fixture directories failing security audits | Enforced standard `022` umask in test environments. |
| [`packaging/linux/steamos/tests/test-*.sh`](../../packaging/linux/steamos/tests/) | Payload dummy `/usr/bin/env` failed under multicall cargo uutils coreutils | Prefer standalone `gnuenv` binary when available or fallback to `env`. |
| [`tests/unit/platform/linux/test_session_controller.sh`](../../tests/unit/platform/linux/test_session_controller.sh) | 1-second timeout deadline raced with subsecond `SECONDS` increment under parallel test execution | Use 2-second deadline and 3-second boundary assertion to eliminate timing race. |
